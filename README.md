...
Local-first orchestration control plane built around one persistent Chief of Staff. Hive settles reversible calls itself and asks the director only for product judgment and authority calls. The web home shows those calls and one digest of what shipped, what hive decided, and what is stuck since the director last looked; everything else sits behind a single Details menu. Nothing hive does leaves a hive marker in the project: branches, PRs, commits and ticket comments read like a teammate's work. hive owns durable state (SQLite), an event-driven timeline, evidence, policies, and a server-enforced task state machine. See [SPEC.md](./SPEC.md) for the full product spec and [docs/API.md](./docs/API.md) for the HTTP contract.
