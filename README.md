...
- **Autonomy benchmark.** `bun run benchmark:autonomy` creates a disposable release-planning repository, registers it as an isolated Hive project, and gives its supervisor a clear top-level ask. `bun run benchmark:score <workspace>` checks the implementation, evaluator integrity, run completion, verification, retrospective, human interruptions, open decisions, commitments, and decision memos.
