...
`agent` (`"claude" | "codex"`, default `"claude"`; `"codex"` runs the interactive Codex CLI using the machine's ChatGPT login), `agent_mcp` (`"none" | "inherit"`, default `"none"`; `"none"` starts claude workers with `--strict-mcp-config --mcp-config '{"mcpServers":{}}'` so a project- or user-scoped MCP server can never stall a pane on the "New MCP server found" prompt, `"inherit"` opts a project back into its normal MCP config), `codex_model` / `codex_model_by_kind` (optional Codex model overrides; omitted means the current Codex default), `codex_reasoning_effort` / `codex_reasoning_effort_by_kind` (optional Codex reasoning overrides; defaults are `medium` for ship and `low` for scout/chore), `codex_auto_compact_token_limit` (default `64000`), `codex_tool_output_token_limit` (default `6000`), `processed_token_warn` / `processed_token_cap` (defaults `75000000` / `200000000`), `wait_call_warn` / `wait_call_cap` (defaults `25` / `100`; `0` disables a threshold), `agent_argv` (string[], advanced per-project override of the complete command herdr runs; verbatim and responsible for its own briefing),
...
- `review_summary`: the agent's structured self-review, submitted before `ready`. `payload: {done?: string[], iffy?: (string|{what,why})[], decisions?: string[], testing?: string[], followups?: string[], understanding?: {background?: string, scope?: string, essence?: string, walkthrough?: string[], affected_areas?: string[], risk_assessment?: string, participate?: string}}`. The review card presents this mental model before the technical audit. `understanding.check` and `understanding.checks` from agents on an older brief are accepted and ignored.
- `understanding_required`: legacy. Older databases hold it where the director asked to see a change before it ships, and the director hold (`reviewer.ts` `directorHold`) still honors it: that task waits for the director's own Ship. Nothing writes it any more. Older databases also hold `understanding_quiz_attempt`, `understanding_quiz_passed` and `understanding_quiz_deferred` rows from the retired understanding quiz; nothing reads them.
...
On spawn, an accepted intent travels in the agent's brief. It is never written
...
`GET /api/doctor` → `200 {"ok": bool, "checks": [{"name": str, "ok": bool, "required": bool, "detail": str, "fix": str|null}], "update": {"behind": int, "stale": bool, "head": str|null, "error": str|null}}`

The same prerequisite checks as `hive doctor` (git, herdr installed, herdr daemon responding, an agent CLI, `gh`, a writable data dir), memoized for a minute. `ok` is false when a required check fails; `fix` says what to do. `update` is the running checkout against `origin/main`, which the server fetches hourly (`HIVE_ORIGIN_FETCH_MS`); the web app shows a banner for either condition.
...
- `POST /api/tasks/:id/merge` body `{merge_strategy?: "local_ff", override_destructive_check?: boolean, actor?}` → `200 Task` (now `verifying`) | `409` (not `in_review`, a confirmed or unfinished risk check on this head, or the merge failed: conflict / not a fast-forward / gh refused / **destructive auto-rebase**) | `403` (denied by a `task.merge` authority rule) | `404` | `400` (local merge but no `repo_path`/`branch`). The optional director `actor` is recorded on `merged`, `merge_failed`, and `merge_blocked_destructive` events.
  ...
  **Who ships it.** This endpoint is the director's Ship. Hive's own auto-merge lands a settled review without the director only when `directorHold` (`reviewer.ts`) finds nothing a person must decide. It holds for the director when the task kind is outside `config.auto_merge.kinds`, when the automatic review could not read the change or raised a risk or question the per-risk check did not clear, when the reviewed diff touches a sensitive path (`auth`, `token`, `security`, `payment`, `billing`, `migration`, `secret`, `credential`, `password` by default, overridable with `config.understanding_checks.sensitive_paths`), when the director requested changes on the task before, or when a legacy `understanding_required` event is on it.
...
- `GET /api/stats/autonomy?days=<n>&project_id=<id>&reverts=0` → `200 {window, auto_merge_precision, inbox_load, recovery, agreement}`
  ...
  `inbox_load` is `{by_day, totals, per_day}`. `by_day` has one row per calendar day in the window with a count per attention class (`decision`, `checkpoint`, `dialog`, `stale`) plus `total`.
...
Where intents come from: a newly imported Jira ticket drafts one (`source: jira`, `source_ref` = the key) from its title, description and every comment, and the auto-filed work task carries it, so nothing dispatches until a human accepts it. `POST /api/tasks` with a `brief` and no `intent_id`, from the director, records that brief as an intent and accepts it in the same step — the brief is kept verbatim. A Jira comment landing on a ticket whose work has already finished opens a NEW `incident` intent (`source_ref` = the comment id) that supersedes the record the finished work was accepted from.
...
- `POST /api/intents/draft` body `{project_id (required), from_jira? ("WEB-101"), text?}` (exactly one of `from_jira` / `text`) → `201 Intent` | `400` (unknown project, or both/neither given) | `404` (no mirror for that Jira key). Drafts a `draft` intent with no task attached. `from_jira` reads the mirror hive already holds — its title, brief and every imported comment — so it needs no Jira credentials. `text` that already carries the five headings is stored as written; anything else is mapped by one `claude -p` call, and a failed call still records the text under `## Problem`.
- `POST /api/intents/:id/accept` body `{source:"director"}` → `200 Intent` | `404` | `409` (already accepted/superseded, or an unticked bullet remains under `## Open questions` — the error names them). Records `accepted_by`/`accepted_at` and writes an `intent_accepted` event on the linked task. On acceptance the linked task's brief is REGENERATED from the record (Problem → `## Source`, Proposed outcome → `## Deliverable`, Constraints → hard limits and the check, Affected users and systems → the reading list, plus an `intent: <id> accepted by <who> at <ts>` footer). For a `jira`-sourced intent one comment goes back on the ticket through the ordinary outbound path, only when `config.jira.write` is true.
...
`circuit_breaker`, `agent_unreachable`,
...
3. `next`, when the title or brief reads as security work. The words are the
   same list the director hold applies to changed paths (`auth`,
