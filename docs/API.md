...
- `GET /api/understanding-quizzes[?project_id=<id>]` returns `{quizzes}` for required and deferred checks on each task's latest review. Each quiz includes an opaque `version` for stale-read detection, and `quiz_key`: the id the director's pass is recorded under. Correct answers and explanations are omitted. A task on an accepted intent takes its questions from that intent instead (`quiz_key` = the intent id, plus `intent_id` and `intent_slug`); see [Intents](#intents-the-ask-record).
...
**The understanding quiz comes from the intent.** Accepting an intent mints three checks from `## Proposed outcome` and `## Constraints` — what changes for the affected user, which constraint the change must respect, what is explicitly out of scope — by one `claude -p` call, stored once on the intent (`checks_json`). Every review head on that task then asks the SAME quiz, and the director's pass is keyed on the intent id, so a rebase, a conflict merge or a re-emitted review never re-asks it (before this, #2190 answered identical questions three times). Superseding the intent is a new row with a new id, so a changed ask — and only a changed ask — asks again. A failed model call stores nothing and the task keeps today's diff-based checks. On a task with an intent quiz, `understanding.checks` in a `review_summary` is stored as `agent_checks` for the Report view and is not asked. Tasks with no intent are unchanged.
