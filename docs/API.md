...
### Tasks
- `GET /api/tasks?state=&project_id=&test=` → `200 [Task + {evidence_count, spawn_error}, ...]` (newest `updated_at` first; all filters optional). Tasks under a test/ephemeral project (`config.test === true`) are hidden by default; pass `?test=all` to include them.
