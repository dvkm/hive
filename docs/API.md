...
### Tasks
- `GET /api/tasks?state=&project_id=&test=&compact=` → `200 [Task + {evidence_count, spawn_error}, ...]` (newest `updated_at` first; all filters optional). `compact=1` omits task briefs for list/bootstrap clients; fetch `GET /api/tasks/:id` when a brief is needed. Tasks under a test/ephemeral project (`config.test === true`) are hidden by default; pass `?test=all` to include them.
