...
The gardener never lands a task while the director is still deciding on it: a review that waits for the director's own Ship (`directorHold` in `server/src/reviewer.ts`) stays there until the director clicks Ship or answers a gardener card.
