...
// A control-plane path only means "an agent is calling hive" when the command
// actually MAKES a network call. A path on its own is text: source files, docs
// and tests are full of it. The bare-path form used to match, so editing
// `server/test/authority.test.ts` in the agent's own worktree (a Python heredoc
// doing string replacement, hive-2049) classified as authority-rule tampering —
// the category that guards the permission system — and asked the director to
// approve a test refactor. A gate that cries wolf on its most serious category
// erodes the response to a real one.
//
// Key on the network TOOL, never on how the URL is spelled. Requiring a base
// URL adjacent to the path is a lexical accident: a closing quote (`curl
// "$HIVE_URL"/api/authority/rules`) or a variable (`H=$HIVE_URL; curl
// $H/api/authority/rules`) defeats it, and neither changes what the command
// does. A heredoc rewriting a test file invokes no network tool, so it stops
// matching without anyone having to prove where it writes.
const NET_TOOL = String.raw`(?:^|[\s;&|(])(?:curl|wget|http|httpie|xh)\b`;
...
const DANGEROUS_RAW: [RegExp, string][] = [
  ...
  [
    new RegExp(NET_TOOL + String.raw`[\s\S]*\/api\/decisions\/[^\s"'\`]+\/(?:answer|dismiss)`, "i"),
    "hive decision tampering",
  ],
  [new RegExp(NET_TOOL + String.raw`[\s\S]*\/api\/authority\/rules`, "i"), "hive authority-rule tampering"],
