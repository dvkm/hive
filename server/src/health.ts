...
export function taskWithHealth(db: DB, task: any, sidecar?: SidecarReport | null): any {
  ...
  const skip = task.skip_reason && SKIP_REASONS[task.skip_reason]
    ...
    : null;
  return { ...task, display_id: taskIdentifier(db, task), health: computeHealth(db, task), requeued_to, needs_you_since, never_dispatched: neverDispatched(db, task), review_actionable: reviewActionable(db, task), reviewed, skip, sidecar: sidecar !== undefined ? sidecar : latestSidecar(db, task.id) };
}
