...
// since POST /merge awaits runSmokeAfterMerge synchronously before responding.
...
// Post-merge smoke for a task that just entered `verifying`. It runs the checks
// and nothing else: a pass writes evidence, a fail bounces the task back to the
// agent (both inside runSmoke). It does NOT close the task. hive never moves a
// task to done — the director does that after verifying it (HIVE-604). A
// tracking-only row has no hive worktree to smoke, so there is nothing to run.
export async function runSmokeAfterMerge(db: DB, taskId: string, deps: MonitorDeps = {}): Promise<{ ran: boolean; passed: boolean }> {
  if (isTrackingOnlyTask(getTask(db, taskId) ?? {})) return { ran: false, passed: false };
  return runSmoke(db, taskId, deps);
}
