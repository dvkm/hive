...
// delegates values to a provider (macOS Keychain or Bitwarden). Values are
...
export function providerFor(kind: string | undefined, exec: Exec = defaultExec): SecretProvider {
...
export async function resolveProjectSecrets(
  ...
  for (const row of rows) {
    try {
      const value = await providerFor(row.provider, exec).get(projectId, row.name, row.ref);
