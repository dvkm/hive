...
async function postForm(path: string, fields: Record<string, string>, files: File[], method = "POST") {
  ...
  const res = await handler(new Request("http://127.0.0.1" + path, { method, body: fd }));
...
test("steer with an attachment: file stored, absolute path delivered to the agent", async () => {
  ...
  const events = await (await handler(new Request(`http://127.0.0.1/api/tasks/${taskId}/events`))).json();
...
test("task create with attachments: paths appended to the brief under Attachments", async () => {
  ...
  const brief = (await (await handler(new Request(`http://127.0.0.1/api/tasks/${task.id}/brief`))).json()).brief;
...
test("task edit appends attachments to the existing brief", async () => {
  ...
  const res = await handler(new Request(`http://127.0.0.1/api/tasks/${taskId}`, { method: "PUT", body: fd }));
...
test("task edit without files leaves a null brief null", async () => {
  ...
  const res = await handler(new Request(`http://127.0.0.1/api/tasks/${taskId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
test("attachments do not count as evidence", async () => {
  ...
  const detail = await (await handler(new Request(`http://127.0.0.1/api/tasks/${taskId}`))).json();
...
test("brief attachment block round-trips into displayable attachments", async () => {
  ...
  const res = await handler(new Request("http://127.0.0.1" + files[0].url));
  expect(res.status).toBe(200);
...
test("attachments from a later edit surface too", async () => {
  ...
  for (const n of ["one.png", "two.png"]) {
    ...
    await handler(new Request(`http://127.0.0.1/api/tasks/${taskId}`, { method: "PUT", body: fd }));
  }
  const task = await (await handler(new Request(`http://127.0.0.1/api/tasks/${taskId}`))).json();
