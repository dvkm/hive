...
// The waiver trusts ONLY the agent's own worktree. The shared temp roots are
// shared by every agent on this box, so a process whose cwd sits in /tmp is as
// likely to be another agent's dev server as this agent's own.
test("kill <pid> is waived only when the process runs inside the agent's OWN worktree", () => {
  const env = { HOME: "/Users/ada" };
  ...
  const cwds: Record<number, string | null> = {
    4242: wt,
    4243: `${wt}/apps/web`,
    4244: "/private/tmp/claude-501/sess/scratchpad", // shared temp root
    4245: "/tmp/build", // shared temp root
    5555: "/Users/ada/.herdr/worktrees/monorepo/hive-0cb95dc908e3", // ANOTHER agent
    9999: "/Users/ada/projects/monorepo",
    7777: null, // lookup failed / process already gone
  };
  pidLookup.cwd = (pid) => cwds[pid] ?? null;
  try {
    expect(classify("kill 4242", env, wt).decision).toBe("unknown");
    expect(classify("kill -9 4243", env, wt).decision).toBe("unknown");
    expect(classify("kill -s TERM 4242", env, wt).decision).toBe("unknown");
    expect(classify("kill 4242 4243", env, wt).decision).toBe("unknown");
    // shared temp roots prove nothing about ownership → still gated
    expect(classify("kill -TERM 4244", env, wt).decision).toBe("dangerous");
    expect(classify("kill 4245", env, wt).decision).toBe("dangerous");
    // another agent's worktree is exactly what must not die silently
    expect(classify("kill 5555", env, wt).decision).toBe("dangerous");
    // outside any sandbox, unresolvable, or mixed with one that is → still gated
    expect(classify("kill 9999", env, wt).decision).toBe("dangerous");
    expect(classify("kill -9 9999", env, wt).decision).toBe("dangerous");
    expect(classify("kill 7777", env, wt).decision).toBe("dangerous");
    expect(classify("kill 4242 9999", env, wt).decision).toBe("dangerous");
    // no cwd, or a cwd outside any worktree: the owning worktree is unprovable
    expect(classify("kill 4242", env).decision).toBe("dangerous");
    expect(classify("kill 4242", env, "/Users/ada/projects/monorepo").decision).toBe("dangerous");
    // a PID is not a pattern: pkill/killall never take this branch
    expect(classify("pkill 4242", env, wt).decision).toBe("dangerous");
    expect(classify("killall 4242", env, wt).decision).toBe("dangerous");
    expect(classify("sudo kill 4242", env, wt).decision).toBe("dangerous");
    // job control and pidfiles keep working without any PID lookup
    pidLookup.cwd = () => {
      throw new Error("must not resolve a PID here");
    };
    expect(classify("kill %1 2>/dev/null", env, wt).decision).toBe("unknown");
    expect(classify("pkill -F /tmp/claude-501/sess/scratchpad/dev.pid", env, wt).decision).toBe("unknown");
  } finally {
    pidLookup.cwd = real;
  }
});
