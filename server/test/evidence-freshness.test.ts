...
function makeApi(head: { sha: string; tree?: string }) {
...
async function post(handler: ReturnType<typeof makeHandler>, path: string, body: unknown) {
...
test("evidence is stamped with the worktree HEAD sha, and the ready gate rejects stale evidence", async () => {
  ...
  const s = makeApi(head);
  const p = await post(s.handler, "/api/projects", { name: "p", repo_path: "/repo" });
  const t = await post(s.handler, "/api/tasks", { project_id: p.json.id, title: "task", brief: "b" });
  const id = t.json.id;
  await post(s.handler, `/api/tasks/${id}/spawn`, {}); // sets worktree_path + in_progress

  // Evidence captured at the first commit is stamped with that sha.
  const e1 = await post(s.handler, `/api/tasks/${id}/events`, { type: "evidence", note: "shot", kind: "screenshot" });
  ...
  await fileReview(s.handler, id);
  const r1 = await post(s.handler, `/api/tasks/${id}/events`, { type: "ready", pr_url: "https://gh/pr/1" });
  expect(r1.json.task.state).toBe("in_review");

  // Change requested: bounce back, agent pushes a fix (HEAD advances).
  await post(s.handler, `/api/tasks/${id}/request-changes`, { notes: "bump the page size" });
  head.sha = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";

  // Ready without re-capturing: old evidence is stale → HELD in_progress.
  const r2 = await post(s.handler, `/api/tasks/${id}/events`, { type: "ready" });
  expect(r2.json.held).toBe(true);
  expect(r2.json.reason).toBe("stale_evidence");
  const held = await (await s.handler(new Request(`http://127.0.0.1/api/tasks/${id}`))).json();
  expect(held.state).toBe("in_progress");

  // Re-capture against the new commit → gate passes, hands off.
  await post(s.handler, `/api/tasks/${id}/events`, { type: "evidence", note: "fresh shot", kind: "screenshot" });
  expect(evidenceAtSha(s.db, id, head.sha)).toBe(1);
  const r3 = await post(s.handler, `/api/tasks/${id}/events`, { type: "ready" });
...
test("a stale handoff names both commits and the action, and re-emitting after a same-tree commit clears it without an agent", async () => {
  ...
  await post(s.handler, `/api/tasks/${id}/events`, { type: "evidence", note: "test run", kind: "log" });
  ...
  head.tree = "treeB";
  const review = await post(s.handler, `/api/tasks/${id}/events`, {
    type: "review_summary",
    done: ["did the thing"],
  });
  ...
  const r1 = await post(s.handler, `/api/tasks/${id}/events`, { type: "ready" });
  ...
  await post(s.handler, `/api/tasks/${id}/events`, { type: "evidence", note: "fresh run", kind: "log" });
  await fileReview(s.handler, id);
...
test("a genuinely current review is recorded and hands off first time", async () => {
  ...
  await post(s.handler, `/api/tasks/${id}/spawn`, {});
  await post(s.handler, `/api/tasks/${id}/events`, { type: "evidence", note: "test run", kind: "log" });
  const review = await fileReview(s.handler, id);
  expect(review.status).toBe(201);
  const ready = await post(s.handler, `/api/tasks/${id}/events`, { type: "ready", pr_url: "https://gh/pr/9" });
...
test("the caller cannot decide which commit the evidence is stamped with", async () => {
  ...
  const e = await post(s.handler, `/api/tasks/${id}/events`, {
    type: "evidence",
    note: "test run",
    kind: "log",
  ...
  await fileReview(s.handler, id);
  const ready = await post(s.handler, `/api/tasks/${id}/events`, { type: "ready", pr_url: "https://gh/pr/5" });
  expect(ready.json.task.state).toBe("in_review");
});
...
test("evidence filed with no worktree is stored unstamped and says so", async () => {
  ...
  const p = await post(s.handler, "/api/projects", { name: "p", repo_path: "/repo" });
  const t = await post(s.handler, "/api/tasks", { project_id: p.json.id, title: "task", brief: "b" });
  const e = await post(s.handler, `/api/tasks/${t.json.id}/events`, { type: "evidence", note: "log", kind: "log" });
  expect(e.json.evidence.meta.commit_sha).toBeUndefined();
...
test("evidence captured through the ordinary emit path counts toward a recovery attempt", async () => {
  ...
  await post(s.handler, `/api/tasks/${id}/events`, { type: "evidence", note: "proof", kind: "log" });
  await fileReview(s.handler, id);
  const r = await post(s.handler, `/api/tasks/${id}/events`, { type: "ready" });
