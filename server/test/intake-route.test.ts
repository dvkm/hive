...
// The bug: a corebeat braindump filed under hive. The fix must re-route it.
test("re-routes a corebeat braindump away from the requested hive project", () => {
  const db = openDb(":memory:");
  const hive = mkProject(db, "hive", { repo_path: "/Users/david/projects/hive" });
  const corebeat = mkProject(db, "corebeat", {
    repo_path: "/Users/david/projects/corebeat",
  ...
  expect(r.project_id).toBe(corebeat);
...
test("keeps the requested project when text matches nothing", () => {
  ...
  mkProject(db, "corebeat", { keywords: ["coredata"] });
...
// End-to-end through POST /api/intake: the created task lands in corebeat and
...
test("POST /api/intake files the braindump in the routed project", async () => {
  const db = openDb(":memory:");
  const hive = mkProject(db, "hive");
  const corebeat = mkProject(db, "corebeat", { keywords: ["coredata"] });
