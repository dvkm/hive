    ...
    if (path === `/rest/api/3/issue/${key}`)
      return json({
        ...
        fields: {
          ...
          status: { name: "To Do" },
          labels: [],
          assignee: { displayName: "Sohee Kim" },
...
test("a linked ticket exposes the browse URL the board turns into an Open in Jira action", async () => {
  ...
  db.query("UPDATE tasks SET brief = ? WHERE id = ?").run("Assignee: Sohee Kim\n\nTicket", jiraTask);
...
test("a config pointing somewhere not allow-listed reports unconfigured, and never echoes that host", async () => {
  ...
  expect(r.json.browse_url).toMatch(/^https:\/\/corebeat\.atlassian\.net\/browse\/WEB-\d+$/);
