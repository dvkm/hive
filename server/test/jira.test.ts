...
test("credentialTargetAllowed is true ONLY for the exact compiled-in (site, email) pair", () => {
  const bad: [unknown, unknown][] = [
    ["http://example.atlassian.net", ALLOWED_EMAIL], // http: Basic auth would go over the wire as base64, not encryption
    ["https://evil.atlassian.net", ALLOWED_EMAIL],
    ["https://example.atlassian.net.evil.com", ALLOWED_EMAIL],
    ["https://notexample.atlassian.net", ALLOWED_EMAIL],
    ["https://example.atlassian.net@evil.tld/", ALLOWED_EMAIL], // userinfo: host is evil.tld despite reading as the real site
    ["https://@example.atlassian.net", ALLOWED_EMAIL], // empty userinfo: URL parses username/password as "", so the delimiter itself must be checked
    ["https:////@example.atlassian.net", ALLOWED_EMAIL], // WHATWG slash normalization: URL still resolves this to the allowed host with empty userinfo
    ["https://example.atlassian.net:8443", ALLOWED_EMAIL], // a port is a different endpoint
    ["not a url", ALLOWED_EMAIL],
    ["", ALLOWED_EMAIL],
    [null, ALLOWED_EMAIL],
    [ALLOWED_SITE, "someone-else@example.atlassian.net"], // right site, wrong half of the pair
  ];
...
test("jiraConfig hard no-ops (returns null, never throws) on a mutated site, mutated email, or a project_key outside the allowlist", () => {
  ...
  expect(jiraConfig({ jira: { site: ALLOWED_SITE, email: "evil@example.com", project_key: "WEB" } })).toBeNull();
