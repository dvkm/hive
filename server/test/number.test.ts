...
test("backfill assigns numbers to legacy rows in created_at order", () => {
  ...
  try {
    ...
    // 2) Rewind to the pre-v10 (v9) schema: drop the number column, its index and
    //    the auto-assign trigger, so re-opening re-runs the real v10 backfill.
    ...
    db1.exec("PRAGMA user_version = 9");
    ...
    // 4) Reopen → v10 migration runs the real backfill.
