...
test("a review summary states each fact once: repeated bullets are dropped", async () => {
  const db = openDb(":memory:");
...
test("the review_summary template teaches the plain-wording rules for the mental-model fields", async () => {
  const { db, id } = seed();
